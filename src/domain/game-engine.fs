...
type GameEngine(player1Name:string, player2Name:string) =
    do if player1Name = player2Name then raise PlayersHaveSameNameException
    let sourcedLogger = SourcedLogger()
    do sourcedLogger.Debug("Initializing for {player1} vs. {player2}...", player1Name, player2Name)
    ...
    let toPlayerDetails = function | Player1 -> player1Name | Player2 -> player2Name
    let newDeal dealer =
        let nonDealer = otherPlayer dealer
        let dealerName, nonDealerName = toPlayerDetails dealer, toPlayerDetails nonDealer
    ...
    let newGame dealer =
        sourcedLogger.Debug("Starting game ({dealer} is first dealer)...", toPlayerDetails dealer)
        {
            FirstDealer = dealer
            PreviousDeals = []
        ...
        if player = Player1 then
            ...
            | None ->
                ...
                if hand.Count = DEALT_HAND_COUNT then
                    sourcedLogger.Debug("Awaiting cards for crib from {player}...", player1Name)
            ...
            | None ->
                ...
                if hand.Count = DEALT_HAND_COUNT then
                    sourcedLogger.Debug("Awaiting cards for crib from {player}...", player2Name)
    ...
    let handToCrib (currentDeal:CurrentDeal) player forCrib =
        ...
        sourcedLogger.Debug("...{player} adds {forCrib} to crib -> hand is {hand}", toPlayerDetails player, cardsText forCrib, cardsText hand)
        hand
        ...
        | Some peggingState ->
            ...
            let score =
                ...
                | h :: t ->
                    ...
                    match played with
                    | Some played -> sourcedLogger.Debug("...{player} plays {previous}{played} = {runningTotal} -> scores {score}", toPlayerDetails player, previous, played, runningTotal, score)
                    | None -> sourcedLogger.Debug("...{player} claims a go -> scores {score}", toPlayerDetails player, score)
                    h :: t |> List.iter (fun event -> sourcedLogger.Debug("\t{event}", event.Text))
                    peggingScoreEvent.Trigger(player, card, pegged, h :: t)
                    Some score
                | [] ->
                    match played with
                    | Some played -> sourcedLogger.Debug("...{player} plays {previous}{played} = {runningTotal}", toPlayerDetails player, previous, played, runningTotal)
            ...
            if hand.Count > 0 then sourcedLogger.Debug("...{player} now has {cards}", toPlayerDetails player, cardsText hand)
            peggingState
    ...
    let agent = MailboxProcessor<_>.Start(fun inbox ->
        let rec loop () = async {
            match awaitingForCribPlayer1.Value with
            | Some _ -> sourcedLogger.Debug("Awaiting cards for crib from {player}...", player1Name)
            | None -> awaitingForCribInteractive Player1
            match awaitingForCribPlayer2.Value with
            | Some _ -> sourcedLogger.Debug("Awaiting cards for crib from {player}...", player2Name)
            | None -> awaitingForCribInteractive Player2
            match awaitingPegPlayer1.Value with
            | Some pegState -> sourcedLogger.Debug("Awaiting {cardOrGo} from {player}...", (if pegState.Peggable.Count > 0 then "pegging card" else "go"), player1Name)
            | None -> ()
            match awaitingPegPlayer2.Value with
            | Some pegState -> sourcedLogger.Debug("Awaiting {cardOrGo} from {player}...", (if pegState.Peggable.Count > 0 then "pegging card" else "go"), player2Name)
            | None -> ()
            if awaitingCannotPegPlayer1.Value then sourcedLogger.Debug("Awaiting cannot peg from {player}...", player1Name)
            if awaitingCannotPegPlayer2.Value then sourcedLogger.Debug("Awaiting cannot peg from {player}...", player2Name)
            if awaitingNewDealPlayer1.Value then sourcedLogger.Debug("Awaiting new deal request from {player}...", player1Name)
            if awaitingNewDealPlayer2.Value then sourcedLogger.Debug("Awaiting new deal request from {player}...", player2Name)
            if awaitingNewGamePlayer1.Value then sourcedLogger.Debug("Awaiting new game request from {player}...", player1Name)
            if awaitingNewGamePlayer2.Value then sourcedLogger.Debug("Awaiting new game request from {player}...", player2Name)
            ...
            | Interactive (CannotPeg player) ->
                ...
                match currentDeal.PeggingState with
                | Some peggingState ->
                    sourcedLogger.Debug("...{player} cannot peg", toPlayerDetails player)
            ...
            | Interactive (RequestNewDeal player) ->
                ...
                sourcedLogger.Debug("...new deal requested by {name}", toPlayerDetails player)
            ...
            | Interactive (RequestNewGame player) ->
                ...
                sourcedLogger.Debug("...new game requested by {name}", toPlayerDetails player)
            ...
            | Interactive (Quit player) ->
                sourcedLogger.Debug("...{name} has quit", toPlayerDetails player)
                return ()
            | Cut ->
                let currentDeal = gameState.Value.CurrentDeal
                ...
                let currentDeal =
                    ...
                    | Some event ->
                        sourcedLogger.Debug("Cut: {cutCard} -> {dealer} scores {event}", cardText cutCard, toPlayerDetails currentDeal.Dealer, event.Text)
            ...
            | Pegging ->
                ...
                | Some peggingState ->
                    if peggingState.Completed then
                        sourcedLogger.Debug("Pegging: {player} scored {score}", player1Name, peggingState.Player1Score)
                        sourcedLogger.Debug("Pegging: {player} scored {score}", player2Name, peggingState.Player2Score)
                        ...
                        let peggingState, awaiting =
                            ...
                            | Player1 when peggingState.Player1Hand.Count = 0 && not peggingState.Player2Knocked ->
                                sourcedLogger.Debug("...{player} has no more cards to peg", player1Name)
                                { peggingState with Player1Knocked = true }, None
                            | Player2 when peggingState.Player2Hand.Count = 0 && not peggingState.Player1Knocked ->
                                sourcedLogger.Debug("...{player} has no more cards to peg", player2Name)
                                { peggingState with Player2Knocked = true }, None
                            | player ->
                                let hand = if player = Player1 then peggingState.Player1Hand else peggingState.Player2Hand
                                if hand.Count = 0 then sourcedLogger.Debug("...{player} has no more cards to peg", toPlayerDetails player)
                                ...
                                let canPeg = peggable.Count > 0 || canClaimGo
                                match canPeg with
                                | true -> peggingState, Some (fun () -> (if player = Player1 then awaitingPegPlayer1 else awaitingPegPlayer2).Value <- Some pegState)
                                | false -> peggingState, Some (fun () -> (if player = Player1 then awaitingCannotPegPlayer1 else awaitingCannotPegPlayer2).Value <- true)
                        transact (fun _ ->
                            gameState.Value <- { gameState.Value with CurrentDeal = { currentDeal with PeggingState = Some peggingState } }
            ...
            | ScoreNonDealerHand ->
                ...
                let currentDeal =
                    ...
                    | Some cutCard, None ->
                        ...
                        sourcedLogger.Debug("Hand: {hand} | {cutCard} -> {nonDealer} scores {score}", cardsText hand, cardText cutCard, toPlayerDetails nonDealer, score)
            ...
            | ScoreDealerHand ->
                ...
                let currentDeal =
                    ...
                    | Some cutCard, None ->
                        ...
                        let events = HandScoreEvent.Process(hand, cutCard)
                        let score = events |> List.sumBy (fun event -> event.Score)
                        sourcedLogger.Debug("Hand: {hand} | {cutCard} -> {dealer} scores {score}", cardsText hand, cardText cutCard, toPlayerDetails dealer, score)
                        events |> List.iter (fun event -> sourcedLogger.Debug("\t{event}", event.Text))
            ...
            | ScoreCrib ->
                ...
                let currentDeal =
                    ...
                    | Some cutCard, None ->
                        ...
                        let score = events |> List.sumBy (fun event -> event.Score)
                        sourcedLogger.Debug("Crib: {crib} | {cutCard} -> {dealer} scores {score}", cardsText crib, cardText cutCard, toPlayerDetails dealer, score)
                        events |> List.iter (fun event -> sourcedLogger.Debug("\t{event}", event.Text))
                ...
                sourcedLogger.Debug("...deal finished -> {player1} scored {player1Score} and {player2} scored {player2Score}", player1Name, player1Score, player2Name, player2Score)
                transact (fun _ -> gameState.Value <- { gameState.Value with CurrentDeal = currentDeal })
                if gameState.Value.GameOver then inbox.Post ProcessGame
                else
                    transact (fun _ ->
                        awaitingNewDealPlayer1.Value <- true
                        awaitingNewDealPlayer2.Value <- true)
                    if not awaitingNewDealPlayer1.Value && not awaitingNewDealPlayer2.Value then inbox.Post NewDeal
                return! loop ()
            | NewDeal->
                let currentDeal = gameState.Value.CurrentDeal
            ...
            | ProcessGame ->
                ...
                sourcedLogger.Debug("...game finished -> {player1} {player1Score} - {games2} {player2Score}", player1Name, player1Score, player2Score, player2Name)
                ...
                transact (fun _ ->
                    awaitingNewGamePlayer1.Value <- true
                    awaitingNewGamePlayer2.Value <- true)
                if not awaitingNewGamePlayer1.Value && not awaitingNewGamePlayer2.Value then inbox.Post NewGame
                return! loop ()
            | NewGame ->
