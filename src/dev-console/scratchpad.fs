...
let workInProgress () =
    ...
    let nibsEventText = match NibsScoreEvent.Process(cutCard) with | Some event -> sprintf " -> %s" event.Text | None -> String.Empty
    sourcedLogger.Debug("Cut: {cutCard}{nibsEventText}", cardText cutCard, nibsEventText)
    let hand1Events = HandScoreEvent.Process(hand1, cutCard)
    sourcedLogger.Debug("Hand 1: {hand1} | {cut} -> {score}", cardsText hand1, cardText cutCard, hand1Events |> List.sumBy (fun event -> event.Score))
    hand1Events |> List.iter (fun event -> sourcedLogger.Debug("\t{event}", event.Text))
    let hand2Events = HandScoreEvent.Process(hand2, cutCard)
    sourcedLogger.Debug("Hand 2: {hand2} | {cut} -> {score}", cardsText hand2, cardText cutCard, hand2Events |> List.sumBy (fun event -> event.Score))
    hand2Events |> List.iter (fun event -> sourcedLogger.Debug("\t{event}", event.Text))
    let cribEvents = CribScoreEvent.Process(crib, cutCard)
    sourcedLogger.Debug("Crib: {crib} | {cut} -> {score}", cardsText crib, cardText cutCard, cribEvents |> List.sumBy (fun event -> event.Score))
